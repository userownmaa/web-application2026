...
@login_required
@check_rights(resource_owner_check=True)  
def edit_user(user_id):
    user = User.query.get_or_404(user_id)
    roles = Role.query.all()
    
    can_edit_role = current_user.role and current_user.role.name == 'Администратор'
    ...
    if request.method == 'POST':
        form_data = {
            ...
            'first_name': request.form.get('first_name', '').strip(),
            'middle_name': request.form.get('middle_name', '').strip(),
            'role_id': request.form.get('role_id', '') if can_edit_role else str(user.role_id or '')
        }
